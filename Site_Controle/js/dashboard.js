...
const DashboardPage = (function () {
  ...
  function getCompetencias(lancamentos) {
    return Array.from(
      new Set(
        (lancamentos || [])
          .map((l) => Utils.calculateCompetencia(l.data))
          .filter((c) => c)
      )
    ).sort((a, b) => {
  ...
  function getMostRecentCompetencia(lancamentos) {
  ...
  function setupPeriodFilter() {
    ...
    const initial = getInitialPeriodo(getLancamentos());
    select.value = initial || 'all';
  ...
  function renderCards(totais, periodo, totalRegistros, registrosPeriodo) {
    ...
    if (!totais || totalRegistros === 0) {
      container.textContent = 'Nenhum lançamento disponível. Importe dados para visualizar o dashboard.';
      return;
    }
  ...
  function getSeriesPorMes(periodo) {
    ...
    if (periodo && periodo !== 'all') {
      lancamentos = filterByPeriodo(lancamentos, periodo);
    }
  ...
  function getComparacaoResponsavel(periodo) {
    ...
    return {
      labels: Object.keys(agreg),
      data: Object.values(agreg),
    };
  }
  ...
  function renderCharts(periodo) {
    ...
    if (!totais || totais.totalLancamentosPeriodo === 0) {
      return;
    }

    const datasetGastosCategoria = {
    ...
    charts.gastosCategoria = mountChart('chartGastosCategoria', 'pie', datasetGastosCategoria, { responsive: true, plugins: { legend: { position: 'bottom' } } });
  ...
  function renderRecentAndTop(periodo) {
    ...
    const renderList = (id, list, emptyText = 'Sem dados.') => {
      ...
      head.innerHTML = '<tr><th>Data</th><th>Descrição</th><th>Categoria</th><th>Valor</th></tr>';
  ...
  function renderResumoCategoria(totais) {
    ...
    if (!entries.length) {
      container.textContent = 'Sem gastos no período.';
      return;
    }
    ...
    table.className = 'grid-table small-table';
    const head = document.createElement('thead');
    head.innerHTML = '<tr><th>Categoria</th><th>Total</th></tr>';
    table.appendChild(head);
    const body = document.createElement('tbody');
  ...
  function renderPeriodInfo(periodo, registrosPeriodo, totalRegistros) {
    ...
    if (!totalRegistros) {
      label.textContent = 'Nenhum lançamento cadastrado.';
      return;
    }

    if (periodo === 'all') {
      label.textContent = `Exibindo todos os períodos (${registrosPeriodo} lançamentos).`;
      return;
    }
